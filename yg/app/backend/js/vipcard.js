...
const userId = new URL(document.URL).searchParams.get('userId') || 3;
...
async function render() {
  ...
  try {
    obj = await loadData();
    cards = obj;
    cardIdActionSheet.data = cards.map(element => element.title);
...
async function onCardIdActionSheet(evt) {
  cardIdInput.value = cards[evt.detail].title
}
...
async function onStartDateDatePicker(evt) {
  ...
  startDateInput.dataset.value = evt.detail/1000
}
async function onSubmitBar(evt) {
  if (evt.detail === "1") {
    if (!userId) return;
    console.log(cards, cardIdInput.value, cards.filter(x => x.title === cardIdInput.value));
    const data = {
      card_id: cards.filter(x => x.title === cardIdInput.value)[0].id,
      user_id: userId,
      start_date: startDateInput.dataset.value || 0
    }
    try {
      const response = await fetch(`${baseUri}/v1/admin/vipcard`, {
        method: 'POST',
        headers: {
          "Authorization": window.localStorage.getItem("Authorization")
        },
