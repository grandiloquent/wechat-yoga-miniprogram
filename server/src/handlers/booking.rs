use crate::utils::data::{query_int_with_params, query_json_with_params};
...
pub async fn book(id: i32, openid: String, pool: &State<Pool>) -> Result<String, Status> {
    match pool.get().await {
        Ok(conn) => {
            match query_int_with_params(&conn, "select * from fn_book($1,$2)", &[&id, &openid])
                .await
            {
                Ok(v) => {
                    Ok(v.to_string())
...
pub async fn unbook(id: i32, openid: String, pool: &State<Pool>) -> Result<String, Status> {
    match pool.get().await {
        Ok(conn) => {
            match query_int_with_params(&conn, "select * from fn_unbook($1,$2)", &[&id, &openid])
                .await
            {
                Ok(v) => {
                    Ok(v.to_string())
