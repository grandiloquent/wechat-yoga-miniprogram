...
Page({
    ...
    async loadData() {
        ...
        try {
            ...
            this.setData({
                items: res.map(x => {
                    x.second = `剩余 ${(x.end_date - (new Date().setHours(0, 0, 0, 0) / 1000)) / 86400} 天`
                    x.subhead = `已使用 ${x.count || 0} 次`
                    return x;
                })
            })
        } catch (e) {
...
function fetch(app) {
    return new Promise(((resolve, reject) => {
        wx.request({
            url: `${app.globalData.host}/api/user.query.vip?id=${app.globalData.userInfo.id}`,
            header: {
