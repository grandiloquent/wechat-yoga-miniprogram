...
Page({
    ...
    async onLoad(options) {
        ...
        try {
            ...
            const puzzle = wx.getStorageSync('puzzle')
            if (!puzzle) {
                puzzle = this.sudoku.generate(
                    (difficulty === 1 && "medium") ||
                    (difficulty === 2 && "hard") || "easy"
                );
                wx.setStorageSync('puzzle', puzzle);
    ...
    loadData() {
        const value = wx.getStorageSync('numbers');
        if (value) {
            this.setData({
    ...
    onItemClick(e) {
        ...
        if (this.data.puzzles[index] !== '.') {
            return
        }
        this.setData({ selectedIndex: index });

        this.updateCandidateNumbers(index);
    },
    updateCandidateNumbers() {
        ...
        let indexs = columns.concat(blocks).concat(rows);
        let numbers = [];
    ...
    reset() {
        const difficulty = this.data.difficultySelected;
        const puzzle = this.sudoku.generate(
            (difficulty === 1 && "medium") ||
            (difficulty === 2 && "hard") || "easy"
        );
        wx.setStorageSync('puzzle', puzzle);
